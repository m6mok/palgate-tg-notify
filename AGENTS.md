...
- [rollback.yml](.github/workflows/rollback.yml) — `workflow_dispatch` (input `image_tag`: release version or commit SHA): redeploys an already-built image via deploy.yml without creating tags/releases or moving `latest`. Dispatched from the Actions UI or by the ops bot's `/rollback` and `/release <version>` commands (needs `GITHUB_TOKEN` in the server env file, see [docs/configuration.md](docs/configuration.md)).
